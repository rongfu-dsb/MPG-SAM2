...
def train_one_epoch(model: torch.nn.Module, criterion: torch.nn.Module,
    ...
    print_freq = 10
    for samples, img_evf, targets in metric_logger.log_every(data_loader, print_freq, header):
        samples = samples.to(device)
        img_evf = img_evf.to(device)
        captions = [t["caption"] for t in targets]
        targets = utils.targets_to(targets, device)

        outputs = model(samples, img_evf, captions, targets)
...
def evaluate(model, criterion, postprocessors, data_loader, evaluator_list, device, args):
    ...
    predictions = []
    for samples, img_evf, targets in metric_logger.log_every(data_loader, 10, header):
