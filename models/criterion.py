...
class SetCriterion(nn.Module):
    ...
    def sim(self, inputs, targets, sen_feat, num_boxes):
        """
        Compute the IoU (Intersection over Union) for masks.
        ...
        Args:
            ...
            num_boxes: The number of boxes (for averaging).

        Returns:
            The mean IoU across all boxes.
        """
        # Apply sigmoid to get predicted probabilities for the mask (since inputs are raw logits)
        inputs = inputs.sigmoid()
        # sen_feat = sen_feat.sigmoid()
        # Flatten the inputs and targets for easier computation
        inputs = inputs.flatten(1)  # [batch_size, num_pixels]
        targets = targets.flatten(1)  # [batch_size, num_pixels]

        src_sen_sim = F.cosine_similarity(inputs,sen_feat,dim=1)
        tag_sen_sim = F.cosine_similarity(targets,sen_feat,dim=1)
        # Compute intersection: the overlapping area between inputs and targets
        # intersection = (inputs * targets).sum(1)
        #
        # # Compute union: the total area covered by both inputs and targets
        # union = inputs.sum(1) + targets.sum(1) - intersection
        #
        # # Compute IoU: intersection / union, avoid division by zero with epsilon
        # iou = intersection / union
        result = self.mseloss(src_sen_sim,tag_sen_sim)
        # squared_diff = (iou-src_iou)**2
        # result = squared_diff.sum()/(num_boxes*t)
        # Return the mean IoU across all boxes
        return result

    def loss_sim(self, outputs, targets, indices, num_boxes):
        """Compute the losses related to the masks: the focal loss and the dice loss.
           targets dicts must contain the key "masks" containing a tensor of dim [nb_target_boxes, h, w]
        """
        assert "pred_masks" in outputs
        ...
        # upsample predictions to the target size
        # src_masks = interpolate(src_masks, size=target_masks.shape[-2:], mode="bilinear", align_corners=False)
        src_masks = src_masks.flatten(0,1).flatten(1)  # [bt,hw]

        target_masks = target_masks.flatten(0,1).flatten(1)  # [bt,hw]
        sen_feat = sen_feat.flatten(0,1).flatten(1)
        losses = {
            "loss_sim": self.sim(src_masks,target_masks,sen_feat,num_boxes),
        }
        return losses
