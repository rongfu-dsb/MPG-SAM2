...
class A2DSentencesPostProcess(nn.Module):
    ...
    def forward(self, outputs, orig_target_sizes, max_target_sizes):
        ...
        B, N, _, _ = out_masks.shape
        # out_logits = torch.ones(B, N).to(out_masks.device)
        out_h, out_w = out_masks.shape[-2:]

        # scores = out_logits.sigmoid()
        scores = torch.ones(B, N).to(out_masks.device)
        pred_masks = out_masks
        # pred_masks = F.interpolate(out_masks, size=(out_h * 4, out_w * 4), mode="bilinear",
        #                            align_corners=False)  # [B, N, H, W]
        ...
        for f_pred_masks, resized_size, orig_size in zip(pred_masks, max_target_sizes, orig_target_sizes):
            ...
            f_pred_masks_no_pad = f_pred_masks[:, :f_mask_h, :f_mask_w].unsqueeze(
                1)  # remove the samples' padding, [:, 1, h, w]
            # resize the samples back to their original dataset (target) size for evaluation
            f_pred_masks_processed = F.interpolate(f_pred_masks_no_pad.float(), size=tuple(orig_size.tolist()),
                                                   mode="nearest")  # origin size, [:, 1, h, w]
