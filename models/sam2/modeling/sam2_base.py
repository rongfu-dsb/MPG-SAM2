...
class SAM2Base(torch.nn.Module):
        ...
        if not is_init_cond_frame:
            ...
            if self.use_obj_ptrs_in_encoder:
                ...
                if len(pos_and_ptrs) > 0:
                    ...
                    obj_pos = obj_pos.unsqueeze(1).expand(-1, B, 256)

                    # obj_pos = obj_ptrs.new_zeros(len(pos_list), B, 256).to(obj_ptrs.device)
    ...
    def track_step(
        ...
        obj_memory, obj_memory_pos = self._inter_obj(frame_idx=frame_idx,
                                                     batch_size=current_vision_feats[-1:][-1].size(1),
                                                     is_init_cond_frame=is_init_cond_frame,
                                                     num_frames=num_frames,
                                                     output_dict=output_dict,
                                                     track_in_reverse=track_in_reverse)
