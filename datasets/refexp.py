...
class ModulatedDetection(torchvision.datasets.CocoDetection):
    def __init__(self, img_folder, ann_file, transforms, beit_transforms, return_masks):
        super(ModulatedDetection, self).__init__(img_folder, ann_file)
        self._transforms = transforms
        self.beit_transformers = beit_transforms
    ...
    def __getitem__(self, idx):
        ...
        while not instance_check:
            ...
            if self.beit_transformers is not None:
                target_evf = target
                img_evf, _ = self.beit_transformers(img, target_evf)
        ...
        return img.unsqueeze(0), img_evf.unsqueeze(0),target
...
def make_coco_transforms(image_set, cautious):
    ...
    if image_set == "val":
        return T.Compose(
            [
                T.RandomResize([(1024,1024)], max_size=1024),
                normalize,
            ]
        )
